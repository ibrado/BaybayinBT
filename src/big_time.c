...
const int IMAGE_RESOURCE_IDS[NUMBER_OF_IMAGES] = {
  ...
  RESOURCE_ID_IMAGE_NUM_9, RESOURCE_ID_IMAGE_SUN
};
...
void display_value(unsigned short value, unsigned short row_number, bool show_first_leading_zero) {
  ...
  for (int column_number = 1; column_number >= 0; column_number--) {
	...
	if ((value > 0) && !((value == 0) && (column_number == 0) && !show_first_leading_zero)) {
...
void display_time(PblTm *tick_time) {
  ...
  display_value(tick_time->tm_min, 1, false);
	
  // Draw "sun"	at midnight
  if((tick_time->tm_hour == 0) && (tick_time->tm_min  == 0)) {
	load_digit_image_into_slot(1, 10);
  }
}
